...
class sfCryptoCaptcha
{
  ...
  private function sendImageToBrowser()
  {
    ...
    if(strtoupper($this->config['format']) == 'JPG' || strtoupper($this->config['format']) == 'JPEG')
    {
      ...
      {
        sfContext::getInstance()->getResponse()->setContentType("image/jpeg");
        sfContext::getInstance()->getResponse()->setContent(imagejpeg($this->image, '', 80));
      }
      else
      {
        return false;
    ...
    if(strtoupper($this->config['format']) == 'GIF')
    {
      ...
      {
      	sfContext::getInstance()->getResponse()->setContentType('image/gif');
        sfContext::getInstance()->getResponse()->setContent(imagegif($this->image));
      }
      else
      {
        return false;
    ...
    if(strtoupper($this->config['format']) == 'PNG')
    {
      if(imagetypes() & IMG_PNG)
      {
        ...
        sfContext::getInstance()->getResponse()->setContent(imagepng($this->image));
      }
      else
      {
        return false;
